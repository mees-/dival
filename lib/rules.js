...
module.exports = rules = {
  ...
  'depends':{
    ...
    validate(value, settings) {
      ...
      if (!settings.types.field(dependant)) {
  ...
  'child':{
    ...
    apply(ruleValue, data, settings) {
      let parsedPath = settings.path.split('.');
      let childTemplate = settings.template;
      for (let step of parsedPath) {
        childTemplate = childTemplate[step];
      }
      let outcome = settings.validate(childTemplate, settings.fullData, data, settings.path);

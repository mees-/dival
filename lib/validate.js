  ...
  if (template.templateType === 'object') {
    ...
    for (let fieldName in template) {
      ...
      for (let rule in field) {
        ...
        let outcome = template.rules[rule].apply(field[rule], testData[fieldName],{types:template.types, fullData:data, template, validate, path:`${templatePath}${fieldName}.${rule}`});
    ...
    let field = template[0];
    for (let element of testData) {
      for (let rule in field) {
        if (field[rule] === undefined) {
          continue;
        }
        let outcome = template.rules[rule].apply(field[rule], element, {types:template.types, fullData:data, template, validate, path:`${templatePath}field.${rule}`});
